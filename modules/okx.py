...
class Okx(Starknet):
    ...
    async def okx_withdraw(self, min_amount, max_amount, token_name, network, terminate=True):
        ...
        logger.info(f'[{self._id}][{self.address_str}] Start withdrawal from OKX {amount} {token_name}')
        curr_balance = await self.account.get_balance()
        ...
        try:
            await exchange.withdraw(token_name, amount, self.address_str,
                                    params={
        ...
        while curr_balance == await self.account.get_balance():
            await asyncio.sleep(60)

        logger.success(f'[{self._id}][{self.address_str}] Successfully withdrawn {amount} {token_name} ')
